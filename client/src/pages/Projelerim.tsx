...
export default Projelerim;import React, { useState, useEffect } from "react";
  ...
  return (
    <div className="group bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl hover:border-blue-200 transition-all duration-300 border border-gray-100 flex flex-col h-full relative overflow-hidden">
      ...
      <div className="mt-8 pt-5 border-t border-gray-50 flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 bg-gray-900 hover:bg-blue-600 text-white px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 shadow-md active:scale-95">
          İncele
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3" />
          </svg>
...
const Projelerim: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-16 min-h-screen bg-[#FAFAFB]">
      <header className="text-center mb-14">
        <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight mb-3">Projelerim</h2>
        <p className="text-gray-400 text-sm max-w-sm mx-auto">Yönettiğiniz veya dahil olduğunuz aktif işler</p>
      </header>

      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((n) => <div key={n} className="h-60 bg-gray-200/50 animate-pulse rounded-2xl"></div>)}
        </div>
      ) : error ? (
