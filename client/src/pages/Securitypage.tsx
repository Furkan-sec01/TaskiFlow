...
export default function SecurityPage() {
  ...
  const passwordPolicyError = (val: string): string | null => {
    if (val.length < 8) return "Şifre en az 8 karakter olmalıdır.";
    if (!/[A-Z]/.test(val)) return "Şifre en az bir büyük harf içermelidir.";
    if (!/[0-9]/.test(val)) return "Şifre en az bir rakam içermelidir.";
    return null;
  };
  ...
  const handleUpdatePassword = async () => {
    ...
    if (!curPass) {
      setPassError("Mevcut şifrenizi girin.");
      return;
    }
    const policyError = passwordPolicyError(newPass);
    if (policyError) {
      setPassError(policyError);
      return;
    }
    if (newPass !== confPass) {
    ...
    try {
      ...
      if (!res.ok) {
        setPassError(data?.error || data?.message || "Şifre güncellenemedi. Lütfen tekrar deneyin.");
        return;
      }
      ...
      setPassError("Sunucuya bağlanılamadı. Lütfen tekrar deneyin.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-7">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl p-5 mb-4">
        ...
        {passError && (
          <p className="mt-3 text-xs font-medium text-red-600 bg-red-50 border border-red-100 rounded-md px-3 py-2">{passError}</p>
        )}
        {passSuccess && (
          <p className="mt-3 text-xs font-medium text-green-700 bg-green-50 border border-green-100 rounded-md px-3 py-2">{passSuccess}</p>
        )}

        <button
          onClick={handleUpdatePassword}
          disabled={saving}
          className="mt-4 px-5 py-2 rounded-lg bg-[#4F6EF7] text-white text-sm font-medium hover:bg-[#3d5ce0] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
