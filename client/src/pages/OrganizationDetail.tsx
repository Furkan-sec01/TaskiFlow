    ...
    <div className={`min-h-screen p-8 transition-colors ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className="max-w-6xl mx-auto flex justify-between items-center mb-10">
        ...
        <div className="flex gap-3">
          ...
          {/* 🚀 DÜZELTME: Ekibi kapat butonu artık doğru fonksiyonu çağırıyor */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {isProjectsLoading ? <div className="col-span-full text-center font-bold opacity-50">Projeler yükleniyor...</div> : projects.map((project) => (
              ...
              <div 
                ...
                className={`group p-8 rounded-[2.5rem] border transition-all cursor-pointer relative hover:shadow-2xl hover:-translate-y-2 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100 shadow-sm'}`}
              >
                ...
                <div className="flex justify-between items-start mb-8">
                  <div className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-colors ${darkMode ? 'bg-slate-700 group-hover:bg-blue-600' : 'bg-blue-50 group-hover:bg-blue-600 group-hover:text-white'}`}><FolderKanban size={24} /></div>
