...
const Notifications = () => {
  ...
  const markAsRead = (id: string) => {
    ...
    fetch(`http://localhost:5000/api/notifications/${id}/read`, { method: "PATCH", headers: authHeaders() }).catch(console.error);
  };

  const markAllAsRead = () => {
    setNotifications(prev => prev.map(n => ({ ...n, isRead: true })));
    fetch(`http://localhost:5000/api/notifications/read-all`, { method: "PATCH", headers: authHeaders() }).catch(console.error);
  };

  const deleteNotification = (id: string) => {
    setNotifications(prev => prev.filter(n => n.id !== id));
    fetch(`http://localhost:5000/api/notifications/${id}`, { method: "DELETE", headers: authHeaders() }).catch(console.error);
  };
