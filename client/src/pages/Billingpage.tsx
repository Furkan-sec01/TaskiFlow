...
const plans: Plan[] = [
  ...
  },
  {
    ...
    label: "Şirketler",
    badge: "🏢 Kurumsal",
    badgeStyle: "bg-orange-50 text-orange-600 border border-orange-200",
    price: "Özel",
...
function FeatureTag({ tag }: { tag: "Plus" | "AI" }) {
  return tag === "Plus" ? (
    <span className="ml-1.5 inline-block text-[9px] font-semibold px-1.5 py-0.5 rounded-md bg-purple-100 text-purple-600 align-middle">
      Plus
    </span>
  ) : (
    <span className="ml-1.5 inline-block text-[9px] font-semibold px-1.5 py-0.5 rounded-md bg-blue-100 text-blue-600 align-middle">
      AI
    </span>
  );
}
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
      ...
      <div className="flex items-baseline gap-1 mb-3">
        <span className="text-xl font-bold text-gray-800">{stat.current}</span>
        {stat.max && (
          <>
            <span className="text-xs text-gray-400">{stat.unit ? `${stat.unit} /` : "/"}</span>
...
export default function BillingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f9] p-8">

      {/* Page Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Billing</h1>
        <p className="text-sm text-gray-500">Planınızı görüntüleyin, yönetin ve yükseltin.</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-white border border-gray-200 rounded-2xl p-1.5 w-fit mb-6 shadow-sm">
        {plans.map((p) => (
          <button
            key={p.key}
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-5">
            ...
            <div className="text-right">
              <p className={`font-bold text-gray-900 ${plan.price.length > 4 ? "text-xl" : "text-3xl"}`}>
                {plan.price}
                {plan.key !== "corp" && (
          ...
          {/* Meta */}
          <div className="flex gap-8 pb-5 mb-5 border-b border-gray-100">
            {plan.key === "corp" ? (
              <>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-0.5">Sözleşme</p>
                  <p className="text-sm font-semibold text-gray-700">Yıllık</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-0.5">Müşteri Temsilcisi</p>
                  <p className="text-sm font-semibold text-gray-700">Atandı</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-0.5">SLA</p>
                  <p className="text-sm font-semibold text-gray-700">%99.9 uptime</p>
                </div>
              </>
            ) : (
              <>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-0.5">Plan Başlangıcı</p>
                  <p className="text-sm font-semibold text-gray-700">{plan.key === "starter" ? "14 Oca 2025" : "—"}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-0.5">Sonraki Ödeme</p>
                  <p className="text-sm font-semibold text-gray-700">{plan.key === "starter" ? "—" : "1 Nis 2025"}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-0.5">Fatura Yöntemi</p>
                  <p className="text-sm font-semibold text-gray-700">{plan.key === "starter" ? "—" : "Kredi Kartı"}</p>
                </div>
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="bg-green-50 border border-green-100 rounded-2xl p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-green-600 mb-3 flex items-center gap-1.5">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 flex items-center justify-between gap-4">
            <p className="text-sm text-gray-600 leading-relaxed">
              <strong className="text-gray-900">{plan.hint.strong}</strong>
              {plan.hint.text}
            </p>
            <button
      ...
      {/* Usage */}
      <h3 className="text-base font-bold text-gray-800 mb-3">Kullanım Durumu</h3>
      ...
      {/* Billing History */}
      <h3 className="text-base font-bold text-gray-800 mb-3">Fatura Geçmişi</h3>
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              {["Tarih", "Açıklama", "Tutar", "Durum", "Fatura"].map((h) => (
                <th key={h} className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-wider text-gray-400">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {plan.invoices.map((inv, i) => (
              <tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                <td className="px-5 py-3.5 text-sm text-gray-600">{inv.date}</td>
                <td className="px-5 py-3.5 text-sm text-gray-600">{inv.description}</td>
                <td className="px-5 py-3.5 text-sm font-medium text-gray-800">{inv.amount}</td>
