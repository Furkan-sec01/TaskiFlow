...
export default function ProfileScreen() {
  ...
  const [userUsername, setUserUsername] = useState("semra.uzun");
  const [userPhone, setUserPhone] = useState("+90 555 123 45 67");
  const [userBio, setUserBio] = useState("stajyer.");
  const [userDepartment, setUserDepartment] = useState("Mobil Geliştirme");
  const [userLocation, setUserLocation] = useState("Zonguldak, Türkiye");
    ...
    const loadPreferences = async () => {
      try {
        ...
        if (savedNotifications !== null) {
          setNotifEnabled(savedNotifications === "true");
        }
      } catch (error) {
  ...
  const handleNotificationChange = async (value: boolean) => {
    try {
      ...
      console.log("Bildirim ayarı kaydedilemedi:", error);
  ...
  const saveProfile = () => {
    ...
    setUserUsername(tempUsername.trim());
    setUserPhone(tempPhone.trim());
    setUserBio(tempBio.trim());
    setUserDepartment(tempDepartment.trim());
    ...
    return (
      ...
      >
        <Pressable
          ...
          onPress={() => toggleSecuritySection(sectionKey)}
        >
          <View
            style={[
        ...
        <View style={styles.profileCard}>
          ...
          <View style={styles.roleRow}>
            <View style={styles.roleBadge}>
              <Text style={styles.roleBadgeText}>Stajyer</Text>
            </View>
            <View style={styles.roleBadge}>
              <Text style={styles.roleBadgeText}>TaskiFlow Ekibi</Text>
            </View>
          </View>
