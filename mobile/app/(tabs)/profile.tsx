...
const STORAGE_KEYS = {
  ...
  connectedAccounts: "profile_connected_accounts",
};
...
// 🔥 EN KRİTİK
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      try {
        ...
        const savedConnected = await AsyncStorage.getItem(STORAGE_KEYS.connectedAccounts);
        if (savedConnected) setConnectedProviders(JSON.parse(savedConnected));
      } catch (error) {
  ...
  const toggleConnectedProvider = async (provider: string) => {
    try {
      const next = { ...connectedProviders, [provider]: !connectedProviders[provider] };
      setConnectedProviders(next);
      await AsyncStorage.setItem(STORAGE_KEYS.connectedAccounts, JSON.stringify(next));
    } catch (error) {
      console.log("Bağlı hesap kaydedilemedi:", error);
      Alert.alert("Hata", "Bağlı hesap kaydedilemedi.");
    }
  };
  ...
  const saveSecurityPassword = async () => {
    ...
    if (!/[A-Z]/.test(newPass.trim())) {
      Alert.alert("Hata", "Yeni şifre en az bir büyük harf içermeli.");
      return;
    }

    if (!/[0-9]/.test(newPass.trim())) {
      Alert.alert("Hata", "Yeni şifre en az bir rakam içermeli.");
      return;
    }
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          color: "#8B5CF6",
          onPress: () => setConnectedModal(true),
        },
        {
          icon: "credit-card",
      ...
      items: [
        {
          ...
          color: "#8B5CF6",
          onPress: () =>
            Linking.openURL("mailto:support@ndmsoftware.com").catch(() =>
          ...
          return (
            <Pressable
              ...
              style={[
                styles.optionRow,
                { backgroundColor: colors.inputBg },
                isConnected && [styles.optionRowActive, { backgroundColor: colors.cardLight, borderColor: colors.primary }],
              ]}
              onPress={() => toggleConnectedProvider(provider)}
            >
              <Text style={[styles.optionText, { color: colors.text }, isConnected && styles.optionTextActive]}>
                {provider}
              </Text>
              <Text style={{ fontSize: 12, fontWeight: "600", color: isConnected ? "#16A34A" : colors.textSecondary }}>
                {isConnected ? "Bağlı" : "Bağla"}
              </Text>
            </Pressable>
          );
        })}
        ...
        <SafeAreaView style={[styles.securityScreen, { backgroundColor: colors.background }]}>
          ...
          >
            <SecurityAccordionItem
              ...
              subtitle="En az 8 karakter, bir büyük harf ve rakam içermelidir"
            ...
            >
              ...
              <View style={{ position: "relative", justifyContent: "center" }}>
                <TextInput
                  style={[styles.securityInput, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder, color: colors.inputText, paddingRight: 44 }]}
                  value={currentPass}
                ...
                <Pressable
                  ...
                  hitSlop={10}
                >
                  <MaterialIcons name={showCurrentPass ? "visibility-off" : "visibility"} size={20} color={colors.textSecondary} />
                </Pressable>
              </View>
              ...
              <View style={{ position: "relative", justifyContent: "center" }}>
                ...
                <Pressable
                  ...
                  hitSlop={10}
                >
                  <MaterialIcons name={showNewPass ? "visibility-off" : "visibility"} size={20} color={colors.textSecondary} />
                </Pressable>
              </View>
              ...
              <View style={{ position: "relative", justifyContent: "center" }}>
                ...
                <Pressable
                  ...
                  hitSlop={10}
                >
                  <MaterialIcons name={showConfirmPass ? "visibility-off" : "visibility"} size={20} color={colors.textSecondary} />
                </Pressable>
              </View>
